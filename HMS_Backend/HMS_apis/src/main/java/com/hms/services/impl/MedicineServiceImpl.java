...
public class MedicineServiceImpl implements MedicineService {
	...
	/*
	 ...
	 @Autowired
	    private HealthHistoryRepo health;

	 
	@Override
	public MedicineDto createMedicine(MedicineDto medicineDto, Integer healthId) {
		Health_History health = this.health.findById(healthId)
	                .orElseThrow(() -> new ResourceNotFoundException("Patient ", "Patient id",healthId));
	...
	public MedicineDto updateMedicine(MedicineDto medicineDto, Integer medicineId) {
		 ...
		 Health_History health = this.health.findById(medicineDto.getHealthHistory().getHealthId()).get();
	...
	public MedicineResponse getAllMedicine(Integer pageNumber, Integer pageSize, String sortBy, String sortDir) {
		  Sort sort = (sortDir.equalsIgnoreCase("asc")) ? Sort.by(sortBy).ascending() : Sort.by(sortBy).descending();
	        ...
	        List<MedicineDto> medicineDtos = allMedicines.stream().map((medicine) -> this.modelMapper.map(medicine, MedicineDto.class))
	...
	public List<MedicineDto> getMedicineByHealthHistory(Integer healthId) {
		Health_History healthi = this.health.findById(healthId)
	        ...
	        List<MedicineDto>medicineDtos = medicines.stream().map((medicine) -> this.modelMapper.map(medicine, MedicineDto.class))
	                .collect(Collectors.toList());
	...
	public List<MedicineDto> searchMedicine(String keyword) {
		 List<Medicine> medicines = this.medicineRepo.searchByMedicineId("%" + keyword + "%");
	        List<MedicineDto> medicineDtos = medicines.stream().map((medicine) -> this.modelMapper.map(medicine, MedicineDto.class)).collect(Collectors.toList());
	        return medicineDtos;
	}
*/
}
