...
//        patient.setWard(ward);
    ...
    public PatientResponse getAllPatient(Integer pageNumber, Integer pageSize, String sortBy, String sortDir) {
        ...
        List<PatientDto> patientDtos = allPatients.stream().map((patient) -> this.modelMapper.map(patient, PatientDto.class))
	...
	public List<PatientDto> getPatientsByDoctor(Integer doctorId) {
		 Doctor doc = this.doctorRepo.findById(doctorId)
	        ...
	        List<PatientDto> patientDtos = patients.stream().map((patient) -> this.modelMapper.map(patient, PatientDto.class))
	                .collect(Collectors.toList());
	...
	public List<PatientDto> searchPatientById(String keyword) {
		 List<Patient> patients = this.patientRepo.searchByPatientId("%" + keyword + "%");
	        List<PatientDto> patientDtos = patients.stream().map((patient) -> this.modelMapper.map(patients, PatientDto.class)).collect(Collectors.toList());
	        return patientDtos;
	}
